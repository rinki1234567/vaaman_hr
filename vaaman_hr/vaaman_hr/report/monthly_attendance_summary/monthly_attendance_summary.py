...
#             filters.companies.extend(
#                 get_descendants_of("Company", filters.company)
#             )
...
#         (
#             day
#             for shifts in attendance_map.values()
...
#         message += f"""
#             <span style='border-left: 2px solid {color};
#                 padding-right: 12px;
#                 padding-left: 5px;
#                 margin-right: 3px;'>
#                 {status} - {abbr}
...
#     special_items = {
#         "Paid Public Holiday": "PPH",
#         "Holiday Present": "H/P",
#     }

#     for label, abbr in special_items.items():
#         message += f"""
#             <span style='border-left: 2px solid #8e44ad;
#                 padding-right: 12px;
#                 padding-left: 5px;
#                 margin-right: 3px;'>
#                 {label} - {abbr}
...
#             },
#             {
#                 "label": _("Staff/Worker"),
#                 "fieldname": "custom_staffworker",
#                 "fieldtype": "Data",
#                 "width": 110,
#             },
#             {
#                 "label": _("Get Pass Number"),
...
#         # --------------------------------------------------------------
#         # SEPARATE COMPENSATORY OFF COLUMN
#         # --------------------------------------------------------------
...
#     """
#     Creates dynamic columns for Leave Types.

#     Compensatory Off is intentionally excluded because
#     it has its own separate column.
#     """
...
#         # Compensatory Off has separate column
#         if entry == "Compensatory Off":
#             continue
...
#         or (
#             joining.year == filter_year
#             and joining.month > filter_month
#         )
#     ):
...
#     if (
#         relieving.year < filter_year
#         or (
#             relieving.year == filter_year
#             and relieving.month < filter_month
#         )
#     ):
#         return 0
...
#     filters: Filters,
#     attendance_map: dict,
#     last_marked_day: int = 0,
# ) -> list[dict]:

#     employee_details, group_by_param_values = (
#         get_employee_related_details(filters)
#     )
...
#     attendance_list = get_attendance_records(
#         filters
#     )
...
#     # Leave is applicable to all shifts
...
# def get_employee_related_details(
#     filters: Filters,
# ) -> tuple[dict, list]:

#     Employee = frappe.qb.DocType(
#         "Employee"
#     )
...
#         for parameter, employees in groupby(
#             sorted(
#                 employee_details,
#                 key=group_key,
#             ),
...
#                 "custom_staffworker": details.custom_staffworker,
#                 "attendance_device_id": details.attendance_device_id,
#             }
...
#                     last_marked_day,
#                     details.branch, 
#                     details.custom_staffworker
#                 )
#             )

#             if not attendance_for_employee:
#                 continue
...
#             {"": {}},
#             holidays,
#             last_marked_day=last_marked_day,
#         )
#     )
...
#     return {
#         "total_present": res.get(
#             "total_present",
#             0,
#         ),

#         "total_leaves": res.get(
#             "total_leaves",
#             0,
#         ),

#         "total_absent": res.get(
#             "total_absent",
#             0,
#         ),

#         "total_holidays": res.get(
#             "total_holidays",
#             0,
#         ),

#         "total_weekly_off": res.get(
#             "total_weekly_off",
#             0,
#         ),
...
# def get_attendance_summary_and_days(
#     employee: str,
#     filters: Filters,
# ) -> tuple[dict, list]:
...
#     effective_start = get_effective_start_day(
#         date_of_joining,
#         filters,
#     )

#     effective_end = get_effective_end_day(
#         relieving_date,
#         filters,
#     )
...
#             "from_date": [
#                 "<=",
#                 f"{filters.year}-{filters.month}-{total_days}",
#             ],
#             "to_date": [
#                 ">=",
#                 f"{filters.year}-{filters.month}-01",
#             ],
#         },
#         fields=[
#             "from_date",
...
#                 if abbr not in leave_day_map.get(
#                     curr.day,
#                     [],
#                 ):

#                     leave_day_map.setdefault(
#                         curr.day,
#                         [],
#                     ).append(abbr)

#             curr = frappe.utils.add_days(
#                 curr,
#                 1,
#             )
...
#         # SUMMARY
#         # ----------------------------------------------------------
#         paid_days, paid_days_extra_days = calculate_paid_days( branch, staff_worker, t_p, t_l, t_wo, t_h, t_pph,total_days)
        
#         existing_overtime = get_total_overtime(employee, filters)
...
#                 # "total_overtime": get_total_overtime(employee,filters,) + (paid_days_extra_days*8),
#                 "total_overtime": existing_overtime + (paid_days_extra_days*8),
#                 "paid_days": paid_days,

#                 "additinal_ot": get_additional_ot(
#                     employee,
#                     filters,
#                 ),
...
#         },
#         [
#             "payment_days_formula",
#             "custom_month_days_based",
#         ],
#         as_dict=True,
...
#     if formula:
#         values = { "P": t_p, "PL": t_l, "WO": t_wo, "H": t_h, "PPH": t_pph,}
        
    
    
#         row_paid_days = safe_eval(formula, None, values)
#     else:
#         row_paid_days = t_p + t_l 
#     extra_days = 0
#     if month_days_based:
#         max_paid_days = total_days - 4

#         if row_paid_days > max_paid_days:
#             extra_days = row_paid_days - max_paid_days
#             paid_days = max_paid_days
#         else:
#             paid_days = row_paid_days
#         return paid_days, extra_days
...
#         # Sick Leave - Zinc -> Sick Leave
#         if app.leave_type == "Sick Leave - Zinc":

#             l_type_name = "Sick Leave"

#         else:

#             l_type_name = app.leave_type
...
#         if l_type_name == "Compensatory Off":

#             lt_key = "compensatory_off"
...
#         # Separate Compensatory Off column
#         if current_type == "Compensatory Off":
...
#         total_leaves_on_day = 0
#         total_present_on_day = 0

#         for __, attendance_dict in (
#             attendance_map.items()
#         ):

#             for __, attendance in (
...
#     to_date = (
#         f"{filters.year}-{filters.month}-"
#         f"{get_total_days_in_month(filters)}"
#     )
...
def get_columns(filters: Filters) -> list[dict]:
    ...
    if filters.summarized_view:
        ...
        {"label": "Total  Overtime", "fieldname": "total_overtime", "fieldtype": "Float", "width": 150},
        {"label": _("Additional OT"), "fieldname": "additinal_ot", "fieldtype": "Float", "width": 140},
        {"label": _("Paid Days"),"fieldname": "paid_days", "fieldtype": "Float", "width": 140  },
        ])
    return columns
    ...
    for employee, details in employee_details.items():
        ...
        if filters.summarized_view:
            ...
            attendance_for_employee = get_attendance_status_for_detailed_view(
                employee, filters, employee_attendance, holidays, details.date_of_joining, details.relieving_date, last_marked_day, details.branch, details.custom_staffworker
            )
...
def get_attendance_status_for_detailed_view(
    employee: str, filters: Filters, employee_attendance: dict, holidays: list, date_of_joining=None, relieving_date=None,last_marked_day: int = 0,branch=None, staff_worker=None
    ...
    for shift, status_dict in employee_attendance.items():
        ...
        for day in range(1, total_days + 1):
            ...
            if day_att:
                ...
                m_leave_abbr = leave_type_abbr.get(m_leave_type, "L") if m_leave_type else ""
        ...
        existing_overtime = get_total_overtime(employee, filters)
        existing_additional_ot = get_additional_ot(employee, filters)
        row.update({
            "total_present": t_p, 
            "total_leaves": t_l, 
            "total_absent": t_a,
            ...
            "unmarked_days": t_un ,
            "total_overtime": existing_overtime + (paid_days_extra_days*8),
            "additinal_ot": existing_additional_ot,    #new 
            "paid_days": paid_days,   #new
